...
def plot_bus_voltage_histories(iteration_history_A, iteration_history_B, iteration_history_C, iteration_history_D, iteration_history_E, tol):
    """
    Plot voltage magnitude (V pu) vs iteration for each bus
    using the per-iteration history arrays.

    Adds an Iteration 0 point:
        - Slack bus (Type == "SL"): V = 0.98 pu
        - All other buses:         V = 1.00 pu

    Assumes each iteration_history_* is a list of dicts like:
        {
            "Bus": ...,
            "Name": ...,
            "Type": "SL" | "PQ" | "PV",
    ...
    """
    histories = [
        ("Bus A", iteration_history_A),
        ("Bus B", iteration_history_B),
        ("Bus C", iteration_history_C),
        ("Bus D", iteration_history_D),
        ("Bus E", iteration_history_E),
    ]
    ...
    for label, hist in histories:
        if not hist:
            continue  # skip if this bus has no history entries

        # Determine initial V for iteration 0 based on bus type
        bus_type = hist[0].get("Type", "").upper()
        if bus_type == "SL":
            V0 = 0.98
        else:
            V0 = 1.0

        # Existing history: Iter starts at 1, 2, 3, ...
        iters_hist = [entry["Iter"]   for entry in hist]
        volts_hist = [entry["V (pu)"] for entry in hist]

        # Prepend iteration 0
        iters = [0] + iters_hist
        volts = [V0] + volts_hist

        plt.plot(iters, volts, marker="o", label=f"{hist[0]['Name']} ({label})")

    plt.xlabel("Iteration")
    plt.ylabel("Voltage Magnitude (pu)")
    plt.title(f"Bus Voltage Convergence vs Iteration At ε= {tol}")
